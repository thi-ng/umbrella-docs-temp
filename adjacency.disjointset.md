...
|  [subsets()](./adjacency.disjointset.subsets.md) |  | Returns a <code>Map</code> of all subsets (connected components) with their canonical tree root IDs as keys and arrays of node IDs as values. If only the number of subsets is required, use the <code>count</code> property of this class instance instead (O(1), updated with each call to <code>union()</code>). |

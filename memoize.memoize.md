<!-- Do not edit this file. It is automatically generated by API Documenter. -->

[Home](./index.md) &gt; [@thi.ng/memoize](./memoize.md) &gt; [memoize](./memoize.memoize.md)

## memoize() function

Function memoization for arbitrary argument counts. Returns augmented function, which uses the given `Map` implementation to obtain and store memoized result of given args. Supports generics for up to 4 args (otherwise untyped).

\*\*Important:\*\* It only makes sense to use `Map` types which support value (rather than object) equality, e.g. those provided by . Using a native `Map` type here will lead to memory leaks! Alternatively, use .

<b>Signature:</b>

```typescript
export declare function memoize<A, B>(fn: Fn1<A, B>, cache: MapLike<A, B>): Fn1<A, B>;
```

## Parameters

|  Parameter | Type | Description |
|  --- | --- | --- |
|  fn | <code>Fn1&lt;A, B&gt;</code> |  |
|  cache | <code>MapLike&lt;A, B&gt;</code> |  |

<b>Returns:</b>

`Fn1<A, B>`


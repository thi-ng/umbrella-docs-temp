...
|  [alphaNum](./fsm.alphanum.md) | Combination of <code>digit()</code> and <code>alpha()</code>. |
...
|  [lit](./fsm.lit.md) |  |
|  [never](./fsm.never.md) | Returns a matcher which always fails (produces a <code>Match.FAIL</code> result) for any given input. Use <code>always()</code> for the opposite effect. |
...
|  [str](./fsm.str.md) | String-only version of <code>seq()</code>. Returns <code>Match.FULL</code> once the entire given string could be matched. Unless <code>collect</code> is true (default: false), only matches given string and does not collect input. Therefore then also only passes an empty string to <code>fail</code> callback. If <code>collect</code> is true, the failed callback will be called with the collected input. |
|  [until](./fsm.until.md) | Generic array version of <code>untilStr()</code>. untilStr |
|  [untilStr](./fsm.untilstr.md) | String only. Returns a matcher which consumes input until the given string could be matched. If successful, calls <code>callback</code> with string recorded so far (excluding the matched terminator string) and returns <code>Match.FULL</code> result. Else <code>Match.PARTIAL</code>. until |

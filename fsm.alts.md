...
Returns a composed matcher which applies inputs to all given child matchers (`opts`<!-- -->) until either all have failed or one of them returns a full match. If successful, calls `callback` with the context, the child matcher's result and an array of all processed inputs thus far. The result of `alts` is the result of this callback (else undefined).

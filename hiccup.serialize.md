...
If the optional `keys` flag is true (default: false), all elements will have an autogenerated `key` attribute injected. If `span` is enabled, `keys` will be enabled by default too (since in this case we assume the output is meant to be compatible with ).
